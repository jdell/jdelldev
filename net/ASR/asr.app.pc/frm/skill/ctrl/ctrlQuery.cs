    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<Skill> listaVO = (List<Skill>)listObj;

                if (listaVO == null)
                    listaVO = new List<Skill>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(Skill).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Description", typeof(string));
                dt.Columns.Add("objVO", typeof(Skill));

                foreach (Skill obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                Skill objVO = (Skill)this.getRegistroSeleccionado(ref frm);
                lib.bl.skill.doDelete lnSkill = new asr.lib.bl.skill.doDelete(objVO);
                lnSkill.execute();
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                lib.bl.skill.doGetAll lnSkill = new asr.lib.bl.skill.doGetAll();
                List<Skill> ds = lnSkill.execute();
                if (ds == null) ds = new List<Skill>();
