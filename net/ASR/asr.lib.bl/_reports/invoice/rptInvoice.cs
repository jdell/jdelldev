...
{
    public class rptInvoice : Informe
    {
        lib.vo.Invoice _invoice = null;

        public rptInvoice(lib.vo.Invoice invoice)
        {
            _invoice = invoice;
        }
        ...
        protected override object getDataSource()
        {
            try
            {
                ...
                bl.payment.doGetAllByInvoice lnInvoicePayments = new asr.lib.bl.payment.doGetAllByInvoice(_invoice);
                _invoice.Payments = lnInvoicePayments.execute();
        ...
        protected override string getEmbeddedReport()
        {
            try
            {
                ...
                res = "asr.lib.bl._reports.invoice.rdlInvoice.rdlc";
                return res;
            }
            catch (Exception ex)
            {
        ...
        public override void setInformeViewer(ref Microsoft.Reporting.WinForms.ReportViewer viewer)
        {
            try
            {
                ...
                Microsoft.Reporting.WinForms.ReportDataSource rdsInvoicePayments = new Microsoft.Reporting.WinForms.ReportDataSource();
                rdsInvoicePayments.Name = typeof(lib.vo.Payment).FullName.Replace(".", "_");
                rdsInvoicePayments.Value = _invoice.Payments;

                viewer.LocalReport.DataSources.Clear();
                viewer.LocalReport.DataSources.Add(rdsInvoice);
                viewer.LocalReport.DataSources.Add(rdsInvoiceDetail);
                viewer.LocalReport.DataSources.Add(rdsInvoicePayments);
