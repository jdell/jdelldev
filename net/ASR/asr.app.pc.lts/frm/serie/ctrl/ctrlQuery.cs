...
namespace asr.app.pc.lts.frm.serie.ctrl
{
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<Serie> listaVO = (List<Serie>)listObj;

                if (listaVO == null)
                    listaVO = new List<Serie>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(Serie).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Description", typeof(string));
                dt.Columns.Add("objVO", typeof(Serie));

                foreach (Serie obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                Serie objVO = (Serie)this.getRegistroSeleccionado(ref frm);
                lib.bl.serie.doDelete lnSerie = new asr.lib.bl.serie.doDelete(objVO);
                lnSerie.execute();
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                lib.bl.serie.doGetAll lnSerie = new asr.lib.bl.serie.doGetAll();
                List<Serie> ds = lnSerie.execute();
                if (ds == null) ds = new List<Serie>();
