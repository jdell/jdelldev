...
{
    internal class OwnOffice12Renderer : AbstractRenderer
    {
        public OwnOffice12Renderer(DayView dayviewParent)
        {
            this._dayView = dayviewParent;
        }
        ...
        public override void DrawHourLabel(System.Drawing.Graphics g, System.Drawing.Rectangle rect, int hour)
        {
            ...
            {
                if (!((DayView)this._dayView).InvertirHourLabel)
                {
                    ...
                    int offset_Y = this._dayView.HalfHourHeight;

                    rect.Y += offset_Y;
                    g.DrawString("10", MinuteFont, brush, rect);

                    rect.Y += offset_Y;
                    g.DrawString("20", MinuteFont, brush, rect);

                    rect.Y += offset_Y;
                    g.DrawString("30", MinuteFont, brush, rect);

                    rect.Y += offset_Y;
                    g.DrawString("40", MinuteFont, brush, rect);

                    rect.Y += offset_Y;
                    g.DrawString("50", MinuteFont, brush, rect);
                }
                else
                {
        ...
        public override void DrawDayHeader(System.Drawing.Graphics g, System.Drawing.Rectangle rect, DateTime date)
        {
            ...
            if ((topPart.Height > 0) && (topPart.Width > 0) && (lowPart.Height > 0) && (lowPart.Width > 0))
            {
        ...
        public override void DrawAppointment(System.Drawing.Graphics g, System.Drawing.Rectangle rect, Appointment appointment, bool isSelected, int gripWidth)
        {
            ...
            start = appointment.Color;
            end = appointment.Color;
