    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<TipoSintoma> listaVO = (List<TipoSintoma>)listObj;

                if (listaVO == null)
                    listaVO = new List<TipoSintoma>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(TipoSintoma).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Descripcion", typeof(string));
                dt.Columns.Add("objVO", typeof(TipoSintoma));

                foreach (TipoSintoma obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                TipoSintoma objVO = (TipoSintoma)this.getRegistroSeleccionado(ref frm);
                lib.bl.tiposintoma.doDelete lnTipoSintoma = new gesClinica.lib.bl.tiposintoma.doDelete(objVO);
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                ...
                List<TipoSintoma> ds = lnTipoSintoma.execute();
                if (ds == null) ds = new List<TipoSintoma>();
