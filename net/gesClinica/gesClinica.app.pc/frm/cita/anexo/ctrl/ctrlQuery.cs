    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<Anexo> listaVO = (List<Anexo>)listObj;

                if (listaVO == null)
                    listaVO = new List<Anexo>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(Anexo).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Titulo", typeof(string));
                dt.Columns.Add("objVO", typeof(Anexo));

                foreach (Anexo obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                Anexo objVO = (Anexo)this.getRegistroSeleccionado(ref frm);

                //Esta ventana sólo se abre desde la de cita.
                Cita cita = (Cita)((gesClinica.app.pc.frm.cita.frmEditDatosClinicos)_vista.ParentForm).InnerVO;

                cita.Anexo.Sort();
                int index = cita.Anexo.BinarySearch(objVO);
                if (index > -1) cita.Anexo.RemoveAt(index);
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                _vista = (frmQuery)frm;
                //TODO: Docked!
                //lib.bl.anexo.doGetAll lnAnexo = new gesClinica.lib.bl.anexo.doGetAll();
                //List<Anexo> ds = lnAnexo.execute();

                //Esta ventana sólo se abre desde la de cita.
                Cita cita = (Cita)((gesClinica.app.pc.frm.cita.frmEditDatosClinicos)_vista.ParentForm).InnerVO;
                List<Anexo> ds = cita.Anexo;
                if (ds == null) ds = new List<Anexo>();
