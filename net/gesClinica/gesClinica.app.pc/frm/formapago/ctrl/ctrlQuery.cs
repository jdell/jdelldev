    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<FormaPago> listaVO = (List<FormaPago>)listObj;

                if (listaVO == null)
                    listaVO = new List<FormaPago>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(FormaPago).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Descripcion", typeof(string));
                dt.Columns.Add("objVO", typeof(FormaPago));

                foreach (FormaPago obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                FormaPago objVO = (FormaPago)this.getRegistroSeleccionado(ref frm);
                lib.bl.formapago.doDelete lnFormaPago = new gesClinica.lib.bl.formapago.doDelete(objVO);
                lnFormaPago.execute();
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                lib.bl.formapago.doGetAll lnFormaPago = new gesClinica.lib.bl.formapago.doGetAll();
                List<FormaPago> ds = lnFormaPago.execute();
                if (ds == null) ds = new List<FormaPago>();
