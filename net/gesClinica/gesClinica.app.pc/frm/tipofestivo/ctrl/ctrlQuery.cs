    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<TipoFestivo> listaVO = (List<TipoFestivo>)listObj;

                if (listaVO == null)
                    listaVO = new List<TipoFestivo>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(TipoFestivo).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Descripcion", typeof(string));
                dt.Columns.Add("Color", typeof(System.Drawing.Image));
                dt.Columns.Add("objVO", typeof(TipoFestivo));

                foreach (TipoFestivo obj in listaVO)
                {
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                TipoFestivo objVO = (TipoFestivo)this.getRegistroSeleccionado(ref frm);
                lib.bl.tipofestivo.doDelete lnTipoFestivo = new gesClinica.lib.bl.tipofestivo.doDelete(objVO);
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                ...
                List<TipoFestivo> ds = lnTipoFestivo.execute();
                if (ds == null) ds = new List<TipoFestivo>();
