...
{
    internal class ctrlQuerySubCuenta:template.frm.consulta.ctrl.QueryCtrl
    {
        frmQuerySubCuenta _vista = null;
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                dt.Columns.Add("Asiento", typeof(Int32));
                ...
                dt.Columns.Add("Punteado", typeof(Boolean));
                ...
                foreach (Apunte obj in listaVO)
                {
                    ...
                    dr["ID"] = obj.ID;
                    dr["Fecha"] = obj.Fecha;
                    dr["Asiento"] = obj.Asiento.Numero;
                    dr["SubCuenta"] = obj.ContraPartida;
                    dr["Concepto"] = obj.Concepto;
                    dr["Debe"] = obj.Debe;
                    dr["Haber"] = obj.Haber;
                    dr["Saldo"] = acumulado;
                    dr["Punteado"] = obj.Punteado;
                    dr["objVO"] = obj;
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                _vista = (frmQuerySubCuenta)frm;
                List<Apunte> ds = null;
                if ((_vista.cmbEjercicio.SelectedItem != null) && (_vista.cmbSubCuenta.SelectedItem != null))
                {
                    lib.bl.apunte.doGetAllMovimientos lnApunte = new gesClinica.lib.bl.apunte.doGetAllMovimientos((SubCuenta)_vista.cmbSubCuenta.SelectedItem, (Ejercicio)_vista.cmbEjercicio.SelectedItem);
                    ds = lnApunte.execute();
                }
        ...
        public override void filtrarRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                ...
                //***************************
                filtro += string.Format(" AND Fecha >= '{0}' and Fecha < '{1}'", _vista.dateFechaDesde.Value.Date.ToString("dd/MM/yyyy"), _vista.dateFechaHasta.Value.Date.AddDays(1).ToString("dd/MM/yyyy"));
        ...
        public override void setEstiloGridRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                ...
                _vista.dgConsulta.Columns["Debe"].DefaultCellStyle.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
                _vista.dgConsulta.Columns["Debe"].DefaultCellStyle.Format = "c";

                _vista.dgConsulta.Columns["Haber"].DefaultCellStyle.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
                _vista.dgConsulta.Columns["Haber"].DefaultCellStyle.Format = "c";
        ...
        public override void inicializarForm(ref repsol.forms.template.BaseForm frm)
        {
            try
            {
                ...
                // ********************* SubCuenta *********************
                gesClinica.lib.bl.subcuenta.doGetAll lnSubCuenta = new gesClinica.lib.bl.subcuenta.doGetAll();
                List<SubCuenta> listSubCuenta = lnSubCuenta.execute();
                listSubCuenta.Sort(sortSubCuenta);
                _vista.cmbSubCuenta.DataSource = listSubCuenta;
                _vista.cmbSubCuenta.DisplayMember = "FullNameInv";
                _vista.cmbSubCuenta.ValueMember = "Codigo";
                _vista.cmbSubCuenta.SelectedIndex = 0;
        ...
        private int sortSubCuenta(lib.vo.SubCuenta one, lib.vo.SubCuenta two)
        {
            return one.FullNameInv.CompareTo(two.FullNameInv);
        }

        public void loadEjercicio(ref frmQuerySubCuenta frm)
        {
            try
            {
                _vista = (frmQuerySubCuenta)frm;
        ...
        public void Puntear(Apunte apunte, bool punteado)
        {
            try
            {
                apunte.Punteado = punteado;
                lib.bl.apunte.doUpdate lnApunte = new gesClinica.lib.bl.apunte.doUpdate(apunte);
                lnApunte.execute();
            }
            catch (Exception ex)
            {
        ...
        public void PuntearHastaSeleccionado(ref repsol.forms.template.consulta.QueryForm frm, bool punteado)
        {
            try
            {
                _vista = (frmQuerySubCuenta)frm;

                foreach (System.Windows.Forms.DataGridViewRow dr in _vista.dgConsulta.Rows)
                {
                    lib.vo.Apunte apunte = (lib.vo.Apunte)this.DataGridViewRowToVO(dr);
                    Puntear(apunte, punteado);
                    if (dr == _vista.dgConsulta.CurrentRow)
                        break;
                }
            }
            catch (Exception ex)
