...
namespace gesClinica.app.pc.frm._renumerar.ctrl
{
    class ctrlRenumerar:template.frm.edicion.ctrl.EditCtrl
    {
        frmRenumerar _vista = null;
        ...
        public override void guardarDatos(ref repsol.forms.template.edicion.EditForm frm, bool newRecord)
        {
            try
            {
                _vista = (frmRenumerar)frm;
                _vista.Cursor = System.Windows.Forms.Cursors.WaitCursor;

                if (_vista.cmbEjercicio.SelectedItem != null)
                {
                    lib.vo.Ejercicio ejercico = (lib.vo.Ejercicio)_vista.cmbEjercicio.SelectedItem;
                    lib.bl._common.custom.tNUMERABLE tipo;
                    if (_vista.rbAsientos.Checked)
                        tipo = gesClinica.lib.bl._common.custom.tNUMERABLE.ASIENTO;
                    else
                        tipo = gesClinica.lib.bl._common.custom.tNUMERABLE.FACTURA_RECIBIDA;

                    lib.bl.asiento.doRenumerar lnRenumerar = new gesClinica.lib.bl.asiento.doRenumerar(ejercico, tipo);
                    int nElementos = lnRenumerar.execute();
                    template._common.messages.ShowInfo(string.Format("{0} {1}", nElementos, nElementos == 1 ? "elemento renumerado" : "elementos renumerados"), _vista.Text);
                }
            }
            catch (Exception ex)
            {
        ...
        public void loadEjercicio(ref frmRenumerar frm)
        {
            try
            {
                _vista = (frmRenumerar)frm;
