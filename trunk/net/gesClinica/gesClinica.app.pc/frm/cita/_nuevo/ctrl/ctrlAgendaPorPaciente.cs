...
namespace gesClinica.app.pc.frm.cita._nuevo.ctrl
{
    internal class ctrlAgendaPorPaciente : template.frm.BaseCtrl
    {
        ...
        public override void inicializarForm(ref repsol.forms.template.BaseForm frm)
        {
            try
            {
                ...
                _vista.monthCalendar.DateChanged +=new EventHandler(_vista.monthCalendar_DateChanged);

                #region Leyenda Estado Citas
                ...
                _vista.agenda.DataSourceColumns = lnSala.execute();

                this.DateChanged(ref _vista);
        ...
        public void DateChanged(ref frmAgendaPorPaciente frm)
        {
            try
            {
                ...
                _vista.Text = _vista.Paciente.ToString();
                _vista.labFecha.Text = _vista.monthCalendar.Date.ToLongDateString();

                lib.common.tipos.ParDateTime fecha = new gesClinica.lib.common.tipos.ParDateTime();
                fecha.Desde = _vista.monthCalendar.Date;
                fecha.Hasta = _vista.monthCalendar.Date.AddDays(1);

                gesClinica.lib.bl.cita.doGetAllPorFechaYPaciente lnCita = new gesClinica.lib.bl.cita.doGetAllPorFechaYPaciente(fecha, _vista.Paciente);
                _vista.agenda.DataSourceCitas = lnCita.execute();

                //lib.bl.evento.doGetAllPorFecha lnEvento = new gesClinica.lib.bl.evento.doGetAllPorFecha(new lib.common.tipos.ParDateTime(_vista.monthCalendar.Date));
                //_vista.agenda.DataSourceEventos = lnEvento.execute();

                _vista.agenda.Fecha = _vista.monthCalendar.Date;
                _vista.agenda.Paciente = _vista.Paciente;
                _vista.agenda.Load();
            }
            catch (Exception ex)
            {
