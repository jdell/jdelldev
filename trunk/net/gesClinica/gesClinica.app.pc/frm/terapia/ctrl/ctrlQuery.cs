    ...
    internal class ctrlQuery:template.frm.consulta.ctrl.QueryCtrl
    {
        ...
        protected override System.Data.DataView ListVOToDataView(object listObj)
        {
            try
            {
                ...
                List<Terapia> listaVO = (List<Terapia>)listObj;

                if (listaVO == null)
                    listaVO = new List<Terapia>();

                System.Data.DataTable dt = new System.Data.DataTable(typeof(Terapia).FullName);
                dt.Columns.Add("ID", typeof(int));
                dt.Columns.Add("Descripcion", typeof(string));
                dt.Columns.Add("Duracion", typeof(int));
                dt.Columns.Add("Precio", typeof(float));
                dt.Columns.Add("Activo", typeof(bool));
                dt.Columns.Add("objVO", typeof(Terapia));

                foreach (Terapia obj in listaVO)
                {
                    System.Data.DataRow dr = dt.NewRow();
                    dr["ID"] = obj.ID;
                    dr["Descripcion"] = obj.Descripcion;
                    dr["Duracion"] = obj.Duracion;
                    dr["Precio"] = obj.Precio;
                    dr["Activo"] = obj.Activo;
                    dr["objVO"] = obj;
        ...
        public override object BorrarRegistro(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                Terapia objVO = (Terapia)this.getRegistroSeleccionado(ref frm);
                lib.bl.terapia.doDelete lnTerapia = new gesClinica.lib.bl.terapia.doDelete(objVO);
                lnTerapia.execute();
        ...
        public override void ConsultaRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            ...
            {
                _vista = (frmQuery)frm;
                lib.bl.terapia.doGetAll lnTerapia = new gesClinica.lib.bl.terapia.doGetAll();
                List<Terapia> ds = lnTerapia.execute();
                if (ds == null) ds = new List<Terapia>();
        ...
        public override void setEstiloGridRegistros(ref repsol.forms.template.consulta.QueryForm frm)
        {
            try
            {
                ...
                _vista.dgConsulta.Columns["Precio"].AutoSizeMode = System.Windows.Forms.DataGridViewAutoSizeColumnMode.AllCells;
                _vista.dgConsulta.Columns["Duracion"].AutoSizeMode = System.Windows.Forms.DataGridViewAutoSizeColumnMode.AllCells;
                _vista.dgConsulta.Columns["Activo"].AutoSizeMode = System.Windows.Forms.DataGridViewAutoSizeColumnMode.AllCells;

                _vista.dgConsulta.Columns["Precio"].CellTemplate.Style.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
                _vista.dgConsulta.Columns["Duracion"].CellTemplate.Style.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
            }
            catch (Exception ex)
            {
                throw ex;
